...
namespace MapReroll.Compat {
	...
	public static class Compat_ConfigurableMaps {
		private static Assembly cmAssembly;

		public static void Apply(Harmony harmonyInst) {
			try {
				if (GenTypes.GetTypeInAnyAssembly("ConfigurableMaps.HarmonyPatches") == null) {
					return; // mod is not loaded
				}
				Dialog_MapPreviews.DialogOnGUI -= ExtraMapPreviewsDialogOnGUI;
				Dialog_MapPreviews.DialogOnGUI += ExtraMapPreviewsDialogOnGUI;
				GeneratedPreviewPageProvider.PageGenerationStarting -= UpdateConfigs;
				GeneratedPreviewPageProvider.PageGenerationStarting += UpdateConfigs;
				MapRerollController.Instance.Logger.Message("Applied Configurable Maps compatibility layer");
			} catch (Exception e) {
				MapRerollController.Instance.Logger.Error(
					"Failed to apply compatibility layer for Configurable Maps:" + e);
			}
		}
		...
		private static void ExtraMapPreviewsDialogOnGUI(Dialog_MapPreviews previewsDialog, Rect inRect) {
			...
			var buttonRect = new Rect(inRect.width - closeButtonSize.x - elementSpacing - configureButtonSize.x,
				inRect.yMax - configureButtonSize.y, configureButtonSize.x, configureButtonSize.y);
		...
		private static bool TryGetMod(out Mod mod) {
			if (LoadAssembly()) {
				var t = cmAssembly.GetType("ConfigurableMaps.SettingsController");
				if (t != null) {
					try {
						...
						cmAssembly.GetType("ConfigurableMaps.Settings")
							.GetMethod("OpenOnMapSettings", BindingFlags.Public | BindingFlags.Static)
							.Invoke(null, null);
					} catch {
		...
		private static void UpdateConfigs() {
			if (LoadAssembly()) {
				try {
					// ReSharper disable once PossibleNullReferenceException
					cmAssembly.GetType("ConfigurableMaps.DefsUtil")
		...
		private static void Restore() {
			if (LoadAssembly()) {
				try {
					// ReSharper disable once PossibleNullReferenceException
					cmAssembly.GetType("ConfigurableMaps.DefsUtil")
						.GetMethod("Restore", BindingFlags.Public | BindingFlags.Static)
						.Invoke(null, null);
					return;
				} catch {
					// no action
